...
| 5 | AirLLM responde mediante su servicio independiente | ✅ | Servicio `airllm` en compose (perfil `deep`): `NousResearch/Meta-Llama-3.1-8B-Instruct` cargado en `cuda:0`, `/health` en `ready`, generaciones completas (`docs/AIRLLM.md`). Desactivado en la L4 desde el 21-09 (ver criterio 16). |
...
| 23 | Todos los tests pasan | ✅ | Suite de `tests/unit` (142 al 21-09; crece con cada PR), más los 5 del servicio AirLLM; la CI (`.github/workflows/ci.yml`) los ejecuta en cada PR y en cada push a `main`. |
