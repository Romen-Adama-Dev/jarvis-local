...
Usa las herramientas `jarvis_ask`, `jarvis_deep`, `jarvis_job_result`, `jarvis_status`,
`jarvis_models`, `jarvis_disk`, `jarvis_jobs` y `jarvis_cancel_job` (servidor MCP
`jarvis-rag`) para todo lo relacionado con la documentación indexada y el estado de
Jarvis. No uses shell, `exec` ni ninguna otra herramienta genérica para esto: estas
...
* `jarvis_job_result`: recoge el estado o resultado de un trabajo. Úsala cuando el
  usuario pregunte por su consulta profunda o pase un identificador de trabajo.
