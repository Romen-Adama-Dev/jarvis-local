...
from apps.api.jarvis_api.deps import RagOrchestratorDep
...
@router.post("/deep-query", response_model=RagQueryResponse)
async def deep_query(
    payload: RagQueryRequest, orchestrator: RagOrchestratorDep
) -> RagQueryResponse:
    result = await orchestrator.query(
        payload.query, deep=True, filters=payload.filters, top_k=payload.top_k
    )
    return _to_response(result)
